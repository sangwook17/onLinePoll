...
<!-- pollRead.jsp에서 넘어오는 투표 한 항목을 받아서 득표수를 증가시켜 텍스트 파일에 저장한다. -->
<%
//	post 방식으로 데이터가 넘어 올 때 한글 깨짐을 방지한다.
...
//		넘어 온투표 데이터가 숫자인가 검사한다. 
		try {
			int result = Integer.parseInt(temp);
//			넘어 온 투표 데이터가 숫자인 경우 정상적인 투표 범위의 데이터인가 검사하기 위해서 텍스트 파일을 읽어서 투표 항목의 개수를 계산한다.
			String filepath = application.getRealPath("/") + "poll.txt";			
			ArrayList<String> poll = PollRead.pollRead(filepath);
			int itemCount = (poll.size() -1) / 2;
			
//			넘어 온 투표 데이터가 정상적인 투표 범위인가 검사한다. 
			if (result >= 1 && result <= itemCount) {
				
// 				여기까지 왔다는 것은 정상적인 투표 데이터가 넘어왔다는 것이므로 투표한 항목의 투표수를 증가시킨다. 득표수를 1 증가 시킬 위치를 계산한다.
				int index = result + itemCount;

// 				득표수를 증가시킨다. => ArrayList의 index번째 데이터를 1 증가시킨다.
				result =  Integer.parseInt(poll.get(index)) + 1;	

// 				증가 된 득표수를 다시 ArrayList에 저장한다.
...
// 				넘어 온 투표 데이터가 정상적인 투표 범위의 숫자가 아니므로 오류 메시지를 출력하고 pollRead.jsp로 돌려보낸다.			
				out.println("<script>");
...
// 			넘어 온 데이터가 숫자가 아니므로 오류 메시지를 출력하고 pollRead.jsp로 돌려보낸다.
			out.println("<script>");
...
//		jsp는 서버용 스크립트 언어이고 javascipt는 클라이언트용 스크립트 언어이다.
//		하나의 jsp 파일에 서버용 스크립트와 클라이언트용 스크립트를 모두 사용한 경우 서버용 스크립트가 먼저 다 실행되고 난 후 클라이언트용 스크립트가 실행된다. => 코딩 순서와 상관없다.
