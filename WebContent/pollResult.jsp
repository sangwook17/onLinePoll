...
<!-- 일정 시간이 경과되면 현재 페이지로 다시 호출된다. => 새로고침 한다. -->
...
<!-- 텍스트 파일에 저장 된 투표 결과를 읽어와 표시한다. -->
<%
// 투표 결과를 웹 브라우저에 출력하기 위해서 텍스트 파일을 읽어보고 투표 항목의 개수를 계산한다.
	String filepath = application.getRealPath("/") + "poll.txt";			
	ArrayList<String> poll = PollRead.pollRead(filepath);
	int itemCount = (poll.size() -1) / 2;

//	득표율을 계산하기 위해 전체 득표수를 계산한다.			
	int sum = 0;
...
%>
	...
	<tr height="50">
		<!-- colspan 속성으로 열 합기기를 할 수 있고 rowspan 속성으로 행 합치기를 할 수 있다. -->
		<th colspan="2">
			<%=poll.get(0)%>
		</th>
	</tr>
	...
	<tr height="50">
		<td align="right" colspan="2">
			전체 득표수: <%=df1.format(sum)%>
		</td>
	</tr>
<%
	for (int i = 1; i <= itemCount; i++) {
//		ArrayList에 저장 된 득표 수를 얻어오고 득표율을 계산한다.
